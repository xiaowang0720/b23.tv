...
| b站短链备注 | 最后一列「b站短链(备注)」可填可不填,用于标注该链接对应的 B 站短链,仅存档备忘,不影响页面 |
...
| lian4 | 三连 | `pan.quark.cn/s/a59c7b1166af` |
| lian5 | 三连 | `pan.quark.cn/s/4dfb27f9dea7` |
| lian6 | 麻辣烫 | — |
| lian7 | 麻辣烫 | — |
...
- **2026-08-19**:「短链」列更名为「网站路径」,新增可选的「b站短链(备注)」列(placeholder「生成的b站短链」),随配置一并保存,仅作备忘不参与跳转;修复删除错位 bug —— 删除复选框由 `name="delete[]"`(未勾选行不提交,数组下标与行号错位,勾任何一行都会删第一行)改为显式行号 `name="delete[行号]"`;登录成功立即 `session_write_close()` 落盘,消除并发下"登录后掉回登录页"的窗口;同时修复了「重置」按钮不还原删除标记的样式问题。
